...
arp_cache_select(ip_addr_t pa)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        if ((entry->state != ARP_CACHE_STATE_FREE) && (pa == entry->pa)) {
            return entry;
        }
    }
    return NULL;
...
arp_cache_update(ip_addr_t pa, const uint8_t *ha)
{
    ...
    if (!cache) {
        errorf("cache not found: %s", ip_addr_ntop(pa, addr1, sizeof(addr1)));
        return NULL;
    }
