...
net_device_get_iface(struct net_device *dev, int family)
{
    ...
    for (entry = dev->ifaces; entry; entry = entry->next) {
        if (entry->family == family) {
            infof("iface found, family=%d", family);
            return entry;
        }
    }
    errorf("iface not found, family=%d", family);
    return NULL;
}
...
net_run(void)
{
    ...
    if (intr_run() == -1) { // turn interrupt functions on
...
net_shutdown(void)
{
    ...
    intr_shutdown(); // turn interrupt functions down
...
net_init(void)
{
    if (intr_init() == -1) { // initialize interrupt functions
