...
tcp_pcb_release(struct tcp_pcb *pcb)
{
    ...
    debugf("released, local=%s, foreign=%s", ip_endpoint_ntop(&pcb->local, ep1, sizeof(ep1)), ip_endpoint_ntop(&pcb->foreign, ep2, sizeof(ep2)));
...
tcp_pcb_select(struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
        if ((pcb->local.addr == IP_ADDR_ANY || pcb->local.addr) && pcb->local.port == local->port) {
            if (!foreign) {
                return pcb;
            }
...
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
    ...
    debugf("%s => %s, len=%zu (payload=%zu)", ip_endpoint_ntop(local, ep1, sizeof(ep1)), ip_endpoint_ntop(foreign, ep2, sizeof(ep2)), total, len);
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if ((src == IP_ADDR_BROADCAST) || (src == iface->broadcast)) {
        errorf("source is a broadcast address: %s", ip_addr_ntop(src, addr1, sizeof(addr1)));
        return;
    }
    if ((dst == IP_ADDR_BROADCAST) || (dst == iface->broadcast)) {
        errorf("destination is a broadcast address: %s", ip_addr_ntop(dst, addr2, sizeof(addr2)));
        return;
    }
    ...
    hlen = (hdr->off >> 4) << 2;
    seg.seq = hton32(hdr->seq);
    seg.ack = hton32(hdr->ack);
    seg.len = len - hlen;
...
tcp_init(void)
{
    if (ip_protocol_register(IP_PROTOCOL_TCP, tcp_input)) {
