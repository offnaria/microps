...
loopback_isr(unsigned int irq, void *id)
{
    ...
    while (1) {
        ...
        net_input_handler(entry->type, entry->data, entry->len, dev); // call input handler
        memory_free(entry);
    }
    mutex_unlock(&PRIV(dev)->mutex);
    return 0;    
