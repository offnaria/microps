...
ip_dump(const uint8_t *data, size_t len)
{
    ...
    flockfile(stderr); // monopolize stderr 
    hdr = (struct ip_hdr *)data;
    v = (hdr->vhl & 0xf0) >> 4; // ip version
    hl = hdr->vhl & 0x0f; // ip header length (words)
    hlen = hl << 2; // ip header length (bytes)
...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    NET_IFACE(iface)->family = NET_IFACE_FAMILY_IP; // iface == iface->iface
...
ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            errorf("conflicts with already registered type (%u)", type);
            return -1;
        }
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (len < hlen) {
        errorf("shorter than header length");
        return;
    }
    if (len < total) {
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY) {
        ...
        return -1;
    } else { /* NOTE: I'll reqrite this block later. */
        ...
        if (((dst & iface->netmask) != (iface->unicast & iface->netmask)) && (dst != IP_ADDR_BROADCAST)) {
            errorf("unreachable destination: dst=%s", ip_addr_ntop(dst, addr, sizeof(addr)));
            return -1;
        }
    }
    if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
        errorf("too long, dev=%s, mtu=%u < %zu", NET_IFACE(iface)->dev->name, IP_HDR_SIZE_MIN + len);
        return -1;
    }
    id = ip_generate_id();
